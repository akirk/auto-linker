...
The plugin has a settings page at `Settings -> Autolink Text` where a WordPress user can be selected as the bot identity and linked terms can be configured in a table with term, URL, text color, background color, and bold fields. The color fields use WordPress' built-in color picker, and the term field previews the selected styling.
